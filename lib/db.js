...
const FINAL_NOTICE_COLLECTION = 'final_notices';

/**
 * Insert a final notice record.
 * Keyed on teamId (PagerDuty team ID) since scope is resolved from PagerDuty.
 *
 * @param {object} doc
 * @param {string}       doc.teamId
 * @param {string}       doc.teamName
 ...
 * @param {boolean}      doc.dryRun
 */
async function recordFinalNotice({ teamId, teamName, serviceId, serviceName, incidentId, opsgenieRequestId, dryRun }) {
  await db.collection(FINAL_NOTICE_COLLECTION).insertOne({
    teamId,
    teamName,
    serviceId,
    serviceName,
...
/**
 * Returns a Set<teamId> of PagerDuty team IDs that have already received
 * a real (dryRun:false) final notice.
 */
async function getNotifiedFinalNoticeTeamIds() {
  const docs = await db
    .collection(FINAL_NOTICE_COLLECTION)
    .find({ dryRun: false }, { projection: { teamId: 1 } })
    .toArray();
  return new Set(docs.map((d) => d.teamId));
}
