...
const REQUIRED_ENV = ['PAGERDUTY_API_KEY', 'PAGERDUTY_FROM_EMAIL'];
...
function parseArgs() {
  ...
  if (teamFlagIndex !== -1) {
    ...
    if (!candidate || candidate.startsWith('--')) {
      ...
      console.error('  Example: node notifyMigrationComplete.js --team NL-DDP-fundament');
      process.exit(1);
    }
    teamName = candidate;
  }
...
async function runSingleTeam(teamName, dryRun, priorityId) {
  ...
  if (!service) {
    throw new Error(`Team "${team.name}" has no services. Cannot create an incident.`);
  }
  ...
  if (dryRun) {
    ...
    console.log('No incident was created. Run with --execute to create a real incident.');
    return;
  }
  ...
  console.log(`[NOTIFIED] ${team.name} — incident ${incidentId}`);
}
...
async function main() {
  ...
  try {
    ...
    if (!priority) {
      throw new Error(`Priority "${INCIDENT_PRIORITY}" not found in PagerDuty.`);
    }
    ...
    const toNotify  = []; // { team, service }
    const noService = []; // { team }

    for (const team of toProcess) {
      const service = await getFirstServiceForTeam(team.id);
      if (service) {
        toNotify.push({ team, service });
      } else {
        noService.push({ team });
      }
    }
    ...
    if (dryRun) {
      ...
      for (const { team, service } of toNotify) {
        await recordNotification({
          teamId:      team.id,
          teamName:    team.name,
          serviceId:   service.id,
        ...
        console.log(`  [DRY-RUN] Would notify: ${team.name} via service "${service.name}"`);
      }

      console.log(`\n[DRY-RUN] Complete. ${toNotify.length} dry-run record(s) written to MongoDB.`);
      console.log('No incidents were created. Run with --execute to create real incidents.');
      return;
    }
