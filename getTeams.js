...
function printTeam(index, team, matchedTags) {
    ...
    console.log(
        `   Tags:        ${matchedTags.length ? matchedTags.join(', ') : 'N/A'}`
    )
...
function writeReport(rows) {
    ...
    // ── Markdown ──────────────────────────────────────────────────────────────
    const mdPath = path.join(reportsDir, 'teams-report.md')
    const lines = [
    ...
    // ── Excel ─────────────────────────────────────────────────────────────────
    const xlsxPath = path.join(reportsDir, 'teams-report.xlsx')
...
async function main() {
    ...
    if (requestedTagNames.length === 0) {
        ...
        console.log('Teams:')
        console.log('------')
        rows.sort((a, b) => a.team.name.localeCompare(b.team.name)).forEach(
            ({ team, tags }, i) => printTeam(i + 1, team, tags)
        )
