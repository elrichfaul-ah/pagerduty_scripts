...
if (!API_KEY) {
    ...
    console.error(
        'Set it in the .env file or export it as an environment variable:'
    )
...
async function pdGet(path) {
    ...
    if (!response.ok) {
        ...
        throw new Error(
            `PagerDuty API error ${response.status} on ${path}: ${body}`
        )
    }
...
async function checkGlobalOrchestrations() {
    const orchestrations = await fetchAllPages(
    ...
    for (const orch of orchestrations) {
        ...
        const suppressedRules = findSuppressedRules(
            data.orchestration_path.sets
        )
...
function generateMarkdown(serviceFindings, globalFindings, generatedAt) {
    ...
    lines.push(
        `| Global orchestrations with suppression rules | ${totalGlobal} |`
    )
    ...
    if (serviceFindings.length === 0) {
        ...
        for (const f of serviceFindings) {
            for (const r of f.legacyRules) {
                lines.push(
                    `| ${f.serviceName} | ${f.teams} | Legacy Event Rule | ${r.label} |`
                )
            }
            for (const r of f.orchRules) {
                lines.push(
                    `| ${f.serviceName} | ${f.teams} | Service Orchestration | ${r.label} |`
                )
            }
        }
    }
    ...
    lines.push('')
    lines.push(
        '> These are expected Prometheus suppression rules, to be disabled per team as they migrate.'
    )
    lines.push('')
...
async function main() {
    ...
    for (const service of services) {
        const teams =
            service.teams && service.teams.length
    ...
    if (serviceFindings.length > 0) {
        console.log(
            `Services with suppression rules: ${serviceFindings.length}`
        )
        ...
        for (const f of serviceFindings) {
            ...
            if (f.legacyRules.length > 0) {
                console.log(
                    `   Legacy event rules with suppress (${f.legacyRules.length}):`
                )
            ...
            if (f.orchRules.length > 0) {
                console.log(
                    `   Service orchestration rules with suppress (${f.orchRules.length}):`
                )
    ...
    const outputPath = path.join(reportsDir, 'suppressionReport.md')
    fs.writeFileSync(
        outputPath,
        generateMarkdown(serviceFindings, globalFindings, generatedAt)
    )
