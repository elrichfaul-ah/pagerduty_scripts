...
function generateMarkdown(allUsers, pending) {
  ...
  lines.push(`| Pending (not accepted) | ${pending.length} | ${pendingPct}% |`);
  lines.push(`| **Total** | **${total}** | **100%** |`);
  lines.push('');
  lines.push(`> **${pending.length} users** have been sent an invitation and have not yet logged in.`);
  lines.push('');
  lines.push('| # | Name | Teams |');
  lines.push('|---|------|-------|');

  sorted.forEach((u, i) => {
    const teams = u.teams && u.teams.length
...
async function main() {
  ...
  const outputPath = path.join(__dirname, 'pendingInvites.md');
